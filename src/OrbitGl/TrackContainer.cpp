...
void TrackContainer::DrawThreadDependency(PrimitiveAssembler& primitive_assembler,
  ...
  const Color kRedArrow(200, 0, 0, 255);
  const float kOffsetToGetCenterOfThreadStateBar = start_arrow_track->GetThreadStateBarHeight() / 2;
  float x = timeline_info_->GetWorldFromTick(thread_state_slice->begin_timestamp_ns());
  float y_start = start_arrow_track->GetThreadStateBarPos()[1] + kOffsetToGetCenterOfThreadStateBar;
  float y_end = end_arrow_track->GetThreadStateBarPos()[1] + kOffsetToGetCenterOfThreadStateBar;
  ...
  primitive_assembler.AddVerticalArrow(pos, arrow_body_size, arrow_head_size,
                                       IntrospectionWindow::kZValueOverlay, kRedArrow,
                                       arrow_direction);
}
